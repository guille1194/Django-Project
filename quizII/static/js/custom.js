$(document).ready(function() {
    ...
    }else if ($group.data('validate') == "matricula") {
			state = /^\d{6}?$/.test($(this).val());
		}else if ($group.data('validate') == "horas") {
			state = /^[1-8]$/.test($(this).val());
		}else if ($group.data('validate') == "salario") {
      state = /^\d{1,6}(?:\.\d{0,2})?$/.test($(this).val());
    }
